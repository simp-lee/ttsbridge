...
func (a *failingListVoicesAdapter) Capabilities() tts.ProviderCapabilities {
	return testProviderCapabilities(false, tts.AudioFormatMP3)
}

func (a *failingListVoicesAdapter) ListVoices(ctx context.Context, filter tts.VoiceFilter) ([]tts.Voice, error) {
	_ = filter
	_ = ctx
	return nil, a.err
}

func (a *failingListVoicesAdapter) Synthesize(ctx context.Context, request tts.SynthesisRequest) (*tts.SynthesisResult, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}

func (a *failingListVoicesAdapter) SynthesizeStream(ctx context.Context, request tts.SynthesisRequest) (tts.AudioStream, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}
...
func (a *staticVoicesAdapter) Capabilities() tts.ProviderCapabilities {
	return testProviderCapabilities(false, tts.AudioFormatMP3)
}

func (a *staticVoicesAdapter) ListVoices(ctx context.Context, filter tts.VoiceFilter) ([]tts.Voice, error) {
	_ = ctx
	return tts.FilterVoices(a.voices, filter), nil
}

func (a *staticVoicesAdapter) Synthesize(ctx context.Context, request tts.SynthesisRequest) (*tts.SynthesisResult, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}

func (a *staticVoicesAdapter) SynthesizeStream(ctx context.Context, request tts.SynthesisRequest) (tts.AudioStream, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}
...
func (a *blockingListVoicesAdapter) Capabilities() tts.ProviderCapabilities {
	return testProviderCapabilities(false, tts.AudioFormatMP3)
}

func (a *blockingListVoicesAdapter) ListVoices(ctx context.Context, filter tts.VoiceFilter) ([]tts.Voice, error) {
	_ = filter
...
func (a *blockingListVoicesAdapter) Synthesize(ctx context.Context, request tts.SynthesisRequest) (*tts.SynthesisResult, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}

func (a *blockingListVoicesAdapter) SynthesizeStream(ctx context.Context, request tts.SynthesisRequest) (tts.AudioStream, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}
...
func (a *nonCooperativeListVoicesAdapter) Capabilities() tts.ProviderCapabilities {
	return testProviderCapabilities(false, tts.AudioFormatMP3)
}

func (a *nonCooperativeListVoicesAdapter) ListVoices(ctx context.Context, filter tts.VoiceFilter) ([]tts.Voice, error) {
	_ = ctx
	_ = filter
...
func (a *nonCooperativeListVoicesAdapter) Synthesize(ctx context.Context, request tts.SynthesisRequest) (*tts.SynthesisResult, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}

func (a *nonCooperativeListVoicesAdapter) SynthesizeStream(ctx context.Context, request tts.SynthesisRequest) (tts.AudioStream, error) {
	_ = ctx
	_ = request
	return nil, errors.New("not implemented")
}
...
func withTestRegistry(t *testing.T, entries map[string]ProviderFactory) {
	...
	registry = make(map[string]ProviderRegistration, len(entries))
	for name, factory := range entries {
		registry[name] = ProviderRegistration{Factory: factory}
	}
	t.Cleanup(func() {
		registry = original
	})
...
func TestVoicesRun_AllProvidersFailReturnsRuntimeError(t *testing.T) {
	...
	withTestRegistry(t, map[string]ProviderFactory{
		"alpha": func(cfg *ProviderConfig) (tts.Provider, error) {
			_ = cfg
			return &failingListVoicesAdapter{name: "alpha", err: sentinelAlpha}, nil
		},
		"beta": func(cfg *ProviderConfig) (tts.Provider, error) {
			_ = cfg
...
func TestVoicesRun_JSONEmptyResultsEncodesArray(t *testing.T) {
	withTestRegistry(t, map[string]ProviderFactory{
		"alpha": func(cfg *ProviderConfig) (tts.Provider, error) {
			_ = cfg
			return &staticVoicesAdapter{name: "alpha", voices: []tts.Voice{{
	...
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := cmd.Run([]string{"--provider", "all", "--locale", "en-", "--format", "json"}, &stdout, &stderr)
	if err != nil {
		t.Fatalf("Run() unexpected error: %v", err)
	}
...
type timeoutBudgetExpectation struct {
	maxAttemptsArg           string
	minBudgetExclusive       time.Duration
	maxBudgetInclusive       time.Duration
	minElapsedInclusive      time.Duration
	maxElapsedInclusive      time.Duration
	rejectAdapterUnavailable bool
}

func registerBlockingListVoicesProviders(t *testing.T, started chan string, budgets chan time.Duration) {
	t.Helper()
...
func requireRuntimeDeadlineExceeded(t *testing.T, err error) *RuntimeError {
	t.Helper()
...
func assertStartedProviders(t *testing.T, started chan string) {
	t.Helper()
...
func assertTimeoutBudgetsWithinRange(t *testing.T, budgets chan time.Duration, minExclusive, maxInclusive time.Duration) {
	t.Helper()

	for i := 0; i < 2; i++ {
		remaining := <-budgets
		if remaining <= minExclusive || remaining > maxInclusive {
			t.Fatalf("remaining deadline budget = %v, want within (%v, %v]", remaining, minExclusive, maxInclusive)
		}
	}
}

func assertElapsedWithinRange(t *testing.T, elapsed, minInclusive, maxInclusive time.Duration) {
	t.Helper()

	if elapsed < minInclusive || elapsed > maxInclusive {
		t.Fatalf("Run() elapsed = %v, want within [%v, %v]", elapsed, minInclusive, maxInclusive)
	}
}

func assertTimeoutRuntimeWarnings(t *testing.T, runtimeErr *RuntimeError, stderr string) {
	t.Helper()
	...
	if !strings.Contains(stderr, "Warning: failed to list voices from alpha") {
		t.Fatalf("stderr = %q, want alpha warning", stderr)
	}
	if !strings.Contains(stderr, "Warning: failed to list voices from beta") {
		t.Fatalf("stderr = %q, want beta warning", stderr)
	}
}

func runAllProvidersTimeoutBudgetTest(t *testing.T, want timeoutBudgetExpectation) {
	t.Helper()

	started := make(chan string, 2)
	budgets := make(chan time.Duration, 2)
	registerBlockingListVoicesProviders(t, started, budgets)

	cmd := NewVoicesCmd()
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	start := time.Now()
	err := cmd.Run([]string{"--provider", "all", "--http-timeout", "25ms", "--max-attempts", want.maxAttemptsArg}, &stdout, &stderr)
	runtimeErr := requireRuntimeDeadlineExceeded(t, err)

	assertStartedProviders(t, started)
	assertTimeoutBudgetsWithinRange(t, budgets, want.minBudgetExclusive, want.maxBudgetInclusive)
	assertElapsedWithinRange(t, time.Since(start), want.minElapsedInclusive, want.maxElapsedInclusive)
	assertTimeoutRuntimeWarnings(t, runtimeErr, stderr.String())
	if stdout.Len() != 0 {
		t.Fatalf("stdout = %q, want empty", stdout.String())
	}
	if want.rejectAdapterUnavailable && strings.Contains(stderr.String(), "provider adapter unavailable") {
		t.Fatalf("stderr = %q, want timeout warnings only", stderr.String())
	}
}

func TestVoicesRun_AllProvidersShareTimeoutBudget(t *testing.T) {
	runAllProvidersTimeoutBudgetTest(t, timeoutBudgetExpectation{
		maxAttemptsArg:      "1",
		minBudgetExclusive:  0,
		maxBudgetInclusive:  50 * time.Millisecond,
		minElapsedInclusive: 0,
		maxElapsedInclusive: 75 * time.Millisecond,
	})
}
...
func TestVoicesRun_AllProvidersShareScaledTimeoutBudgetWithMaxAttempts(t *testing.T) {
	runAllProvidersTimeoutBudgetTest(t, timeoutBudgetExpectation{
		maxAttemptsArg:           "3",
		minBudgetExclusive:       45 * time.Millisecond,
		maxBudgetInclusive:       100 * time.Millisecond,
		minElapsedInclusive:      50 * time.Millisecond,
		maxElapsedInclusive:      125 * time.Millisecond,
		rejectAdapterUnavailable: true,
	})
}
...
func TestVoicesRun_AllProvidersReturnsOnSharedTimeoutBudgetWhenMultipleProvidersIgnoreCancel(t *testing.T) {
	...
	withTestRegistry(t, map[string]ProviderFactory{
		...
		"gamma": func(cfg *ProviderConfig) (tts.Provider, error) {
			_ = cfg
